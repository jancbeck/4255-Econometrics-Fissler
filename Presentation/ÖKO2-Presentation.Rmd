...
-   Klassisches Regressionsmodel mit Trend $y_{t}=\mu_{t}+u_{t}$
...
## Klassisches Regressionsmodel mit Trend und Season

$$y_{t}=\mu_{t}+\tilde{s}_{t}+\mu_{t}\tilde{s}_{t}+u_{t}$$

```{r, echo=FALSE}

seasonal_trend_model <- tslm(Duration ~ trend + factor(Semester) + trend * factor(Semester), data=ts_weeks)
#summary(seasonal_trend_model)
plot(ts_weeks[,2], xlab="Wochen", ylab="Studienaktivität (h)")
lines(seasonal_trend_model$fitted.values, col="red", las=2)
...
knitr::kable(c, col.names=c("Model1", "Model2"))
```
...
-   Erklärende Variablen:
    ...
    -   Wetter: Sonnenstunden, Temperatur (monatlich, Hohe Warte ZAMG)
